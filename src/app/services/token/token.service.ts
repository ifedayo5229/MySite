  ...
  getSession(): LoginResponseData | null {
    ...
    if (window.localStorage.getItem('AT')) {
      const profiles: Profile = {
        ...
        functionId:      +(localStorage.getItem('FUNCID')   ?? 0),
        locationId:      +(localStorage.getItem('LOCID')    ?? 0),
        locationName:    localStorage.getItem('LOCNAME')    ?? '',
        unitId:          +(localStorage.getItem('UNITID')   ?? 0),
